...
from rmg_gua.gua_peuqse import ct_simulation #sim_wrapper
...
class meoh_plots(): 
    ...
    def __init__(self, project_path, by_species=False, output_species = [], reset_reac=False):

        self.by_species = by_species
        self.output_species = output_species
        self.reset_reac = reset_reac

        self.load_data(project_path, by_species=by_species, output_species=output_species, reset_reac=reset_reac)

    def remap_params(self, parameters, pe_obj, res_path): 
        """
    ...
    def load_data(self,project_path, by_species=False, output_species = [], reset_reac=False):
        """
        ...
        self.results_path  = os.path.join(self.project_path, "config")
        self.param_dict = get_all_param_lists(results_path=self.results_path)
        self.ct_simulation = ct_simulation
        self.ct_simulation.results_path = self.results_path
        
        # these should all be the same, so load it only from one
        with open(os.path.join(self.results_path, "expt_data.yaml"), "r") as f: 
        ...
        pe_object = PEUQSE.load_PE_object(os.path.join(self.project_path, "pe_object.dill"))
        ...
        self.parameters_initial = get_all_param_lists(self.results_path)
        
        # check dim of std prior. if 2-d, take diagonal to get the variance elements
        if len(pe_object.UserInput.std_prior.shape) ==2: 
            self.parameters_stdprior = np.diag(pe_object.UserInput.std_prior)
        else: 
            self.parameters_stdprior = pe_object.UserInput.std_prior
            
        with open(os.path.join(self.project_path, "pickles/mcmc_post_burn_in_statistics.pkl"), "rb") as input_file:
            burn_in_stats = pickle.load(input_file)
            # print(key, ": ", len(burn_in_stats[5]))

        # when we exclude the alphas, we need to remap the parameters to run in cantera
        self.parameters_ct_map = self.remap_params(self.parameters_map, pe_object, self.results_path)
        self.parameters_ct_muaps = self.remap_params(self.parameters_muap, pe_object, self.results_path)
        self.parameters_ct_initial = self.parameters_initial["value_list"]

        # simulate both the most probable and best fit set
        self.ct_simulation.sim_init(self.project_path, output_species = output_species, thermo_by_species=self.by_species, reset_reac=reset_reac)
        self.output_map = self.ct_simulation.simulation_function_wrapper(list(self.parameters_ct_map.values()))

        self.ct_simulation.sim_init(self.project_path, output_species = output_species, thermo_by_species=self.by_species, reset_reac=reset_reac)
        self.output_mu_ap = self.ct_simulation.simulation_function_wrapper(list(self.parameters_ct_muaps.values()))
        
        self.ct_simulation.sim_init(self.project_path, output_species = output_species, thermo_by_species=self.by_species, reset_reac=reset_reac)
        self.output_initial = self.ct_simulation.simulation_function_wrapper(self.parameters_ct_initial)


        # load the lookup dict
        lookup_dict_file = os.path.join(self.results_path, "rmg_2_ck_dict.yaml")
        with open(lookup_dict_file, 'r') as f:
            self.lookup_dict = yaml.load(f, Loader = yaml.FullLoader)

        # load the initial binding energies
        self.yaml_path = os.path.join(self.project_path, "rmg_model", "cantera", "chem_annotated.yaml")
        gas = ct.Solution(self.yaml_path, 'gas')
        surf = ct.Interface(self.yaml_path, 'surface1', [gas])

        self.ct_sltns = {"gas": gas, "surf": surf}

        specs = ['CH3OH','CO','CO2', 'H2', 'H2O']

        # load all of the graaf experimental data
        # generate settings array
        settings_yaml =os.path.join(repo_dir, "rmg_gua", "gua_cantera", "experiments_reorg_onlyopt.yaml")
        with open(settings_yaml, 'r') as f:
            self.all_graaf_expts = yaml.safe_load(f)
        ...
        species_lookup_file = os.path.join(self.results_path, "rmg_2_ck_dict.yaml")
        with open(species_lookup_file, "r") as f: 
            self.label_lookup = yaml.safe_load(f)

        # self.species_labels = []
        # for value in self.output_labels: 
        #     self.species_labels.append(self.label_lookup[value])

        # will update with better math expressions, for now use chemkin names
        self.species_list_pretty = self.label_lookup
    ...
    def get_min_sbr(self, yaml_path):
        """ 
        load cantera solution for map mechanism
        """
        # instantiate sbr object
        sbr = MinSBR(
            yaml_path,
            reac_config=self.all_graaf_expts[0],
            rtol=1.0e-11,
            atol=1.0e-22,
            reaction_list=list(self.parameters_ct_map.values()), 
    ...
    def make_yaml(self):
        ...
        # instantiate sbr object
        sbr = self.get_min_sbr(new_yaml_path)
        
        yml_write = ct.YamlWriter()
        yml_write.set_header(sbr.gas)
        yml_write.add_solution(sbr.gas)
        yml_write.add_solution(sbr.surf)
        yml_write.to_file(os.path.join(self.project_path, "map_mechanism.yaml"))
