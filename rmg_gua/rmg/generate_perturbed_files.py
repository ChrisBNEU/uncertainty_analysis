...
def generate_perturbed_files(
    ...
    sobol_range_map["C"].append((-DELTA_E0_C, DELTA_E0_C))
    sobol_range_map["O"].append((-DELTA_E0_O, DELTA_E0_O))
    sobol_range_map["H"].append((-DELTA_E0_H, DELTA_E0_H))
    sobol_range_map["N"].append((-DELTA_E0_N, DELTA_E0_N))
    sobol_range_map["Vdw"].append((-DELTA_E0_VDW, DELTA_E0_VDW))
    ...
    if len(scaling_groups) > 0:
        ...
        for i in tqdm(range(0, M)):
            ...
            delta_E0_dict = {}
            delta_E0_dict['C'] = float(DELTA_E0_C - 2.0 * x_sobol[i, sobol_map["C"][0]] * DELTA_E0_C)
            delta_E0_dict['O'] = float(DELTA_E0_O - 2.0 * x_sobol[i, sobol_map["O"][0]] * DELTA_E0_O)
            delta_E0_dict['H'] = float(DELTA_E0_H- 2.0 * x_sobol[i, sobol_map["H"][0]] * DELTA_E0_H)
            delta_E0_dict['N']= float(DELTA_E0_N - 2.0 * x_sobol[i, sobol_map["N"][0]] * DELTA_E0_N)

            for atom, be in metal_vals_ref.binding_energies.items():
                metal_vals.binding_energies[atom].value = be.value + delta_E0_dict[atom]
    ...
    if len(thermo_libraries) > 0:
        ...
        for library_key in thermo_database.libraries:
            ...
            for i in tqdm(range(0, M)):
                ...
                delta_E0_vdw = DELTA_E0_VDW  - 2.0 * x_sobol[i, sobol_map["Vdw"][0]] * DELTA_E0_VDW 

                # replace sobol map value with the actual perturbation 
                sobol_map["Vdw"][1][i] = float(delta_E0_vdw)
                sobol_range_map["Vdw"].append((-float(delta_E0_vdw), float(delta_E0_vdw)))
