...
def make_slurm_scripts(
    ...
    if remove_old_slurm_scripts:
        rm_rmg_files = glob.glob(os.path.join(output_path, "rmg_run_scripts/rmg_runs_*.sh"))
        for file in rm_rmg_files:
            try:
                os.remove(file)
            except OSError:
                print(f"file doesn't exist at {file}")

    reference_db = RMG_db_folder
    reference_py = RMG_base_folder
    ...
    perturbed_metal_libs = glob.glob(os.path.join(reference_db, 'input', 'surface', 'libraries', '*_0000.py'))
    ...
    for i in range(0, M, N):
        ...
        # get the metal db as well
        for lib_file in perturbed_metal_libs:
            lib_file_src = lib_file.replace('metal_0000.py', 'metal_${RUN_i}.py')
            file_name_parts = lib_file.split('RMG-database/')
            if len(file_name_parts) != 2:
                raise OSError(f'Bad source metal.py file path {lib_file}')
            lib_file_dest = os.path.join(dest_db_dir, file_name_parts[1].replace('metal_0000.py', 'metal.py'))
            content.append(f'rm "{lib_file_dest}"\n')
            content.append(f'cp "{lib_file_src}" "{lib_file_dest}"\n')
