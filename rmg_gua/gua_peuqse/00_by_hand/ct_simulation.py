...
def simulationFunction(parameters, debug=False):
    ...
    for run in range(0,n_expts): 
        ...
        real_vals_meoh.append(conditions["species_out"]["CH3OH"])
        real_vals_h2o.append(conditions["species_out"]["H2O"])
    ...
    # we want one value for objective function. minimize mse for now
    obj_func = sum((np.array(CH3OH_X) - np.array(real_vals_meoh))**2)/n_expts 
    obj_func += sum((np.array(H2O_X) - np.array(real_vals_h2o))**2)/n_expts
