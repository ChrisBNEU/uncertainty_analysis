...
class MinSBR:
    ...
    def __init__(
        ...
        """
        self.include_expt_unc = include_expt_unc
        ...
        if reaction_list is not None and len(reaction_list) > 5:
            ...
            cat_pert, rxn_pert_dict, thermo_pert_dict = self.load_perts(
                reaction_list, by_species=by_species, include_expt_unc=self.include_expt_unc)
        ...
        # janky, but only do binding energies if len reactionlist ==5
            ...
            self.gas.TP = 298.0, 101325.0
            self.surf.TP = 298.0, 101325.0
            thermo_pert_dict = self.load_perts_beonly(
                reaction_list, by_species=by_species, include_expt_unc=self.include_expt_unc)
        ...
        # catalyst pert has to be class value for resetting the reactor
        self.cat_pert = cat_pert

        # Catalyst Surface Area (pert is 0 if not specified)
        self.cat_area = reac_config['catalyst_area'] + self.cat_pert # [m^3]
    ...
    def load_perts(self, pert_list, by_species=False, include_expt_unc=False):
        """ 
        ...
        count = 0
        if include_expt_unc:
            # for now only looking at uncertainty in the surface area to volume ratio
            # I have tacked it on to the end of the list
            cat_pert = pert_list[-1]
            pert_list_thermoreac = pert_list[:-1]
        else:
            cat_pert = 0.0
            pert_list_thermoreac = pert_list
        if len(pert_list_thermoreac) > 0:
            # print("changing reactions")
            for rule, vals in self.rule_dict_orig.items():
                rule_dict[rule]["A"] = pert_list_thermoreac[count]
                count += 1
                rule_dict[rule]["E0"] = pert_list_thermoreac[count]
                count += 1
                rule_dict[rule]["alpha"] = pert_list_thermoreac[count]
                count += 1
            ...
            if by_species: 
                ...
                assert len(pert_list_thermoreac) == len(self.rule_dict_orig)*3 + len(thermo_pert_dict), "number of reactions in rxn_list does not match number of reactions in rule_dict_orig"

                for num, (key, value) in enumerate(thermo_pert_dict.items()):
                    thermo_pert_dict[key] = pert_list_thermoreac[count]
                    count += 1
                
                
            else: 
                assert len(pert_list_thermoreac) == len(self.rule_dict_orig)*3 + 5, "number of reactions in rxn_list does not match number of reactions in rule_dict_orig"
        ...
        return cat_pert, rule_dict, thermo_pert_dict

    def load_perts_beonly(self, pert_list, by_species=False, include_expt_unc=False):
        """ 
        load the perturbations to a dictionary
        """
        count = 0
    ...
    def run_reactor_transient(self, dt):
        """
        Run single reactor to steady state and save the results to an ordered dictionary in memory

        dt : time increment for advancement
        """
        # setup the results. want a list for each entry that will change. 
        results = {}
        ...
        # transient results
        results['RMG MeOH TOF 1/s'] = []
        results['RMG H2O TOF 1/s'] = []
        
        for i in range(0, len(self.gas.X)):
            results[self.gas.species_names[i]] = []
        for i in range(0, len(self.surf.X)):
            results[self.surf.species_names[i]] = []
        
        # Enter the ROP's
        for i in range(0, len(self.r.kinetics.net_production_rates)):
            results[gas_ROP_str[i]] = []

        for i in range(0, len(self.rsurf.kinetics.net_production_rates)):
            results[surf_ROP_str[i]] = []

        for i in range(0, len(self.rsurf.kinetics.net_rates_of_progress)):
            results[surfrxn_ROP_str[i]] = []

        if gasrxn_ROP_str:
            for i in range(0, len(self.r.kinetics.net_rates_of_progress)):
                results[gasrxn_ROP_str[i]] = []
        ...
        while t < self.time: 
            ...
            results['RMG MeOH TOF 1/s'].append(float(self.molar_flow*(self.r.thermo[self.ch3oh_str].X)/(self.total_sites)))
            results['RMG H2O TOF 1/s'].append(float(self.molar_flow*(self.r.thermo[self.h2o_str].X - self.x_H2O)/(self.total_sites)))

            for i in range(0, len(self.gas.X)):
                results[self.gas.species_names[i]].append(self.gas.X[i])
            for i in range(0, len(self.surf.X)):
                results[self.surf.species_names[i]].append(self.surf.X[i])
            
            # Enter the ROP's
            for i in range(0, len(self.r.kinetics.net_production_rates)):
                results[gas_ROP_str[i]].append(self.r.kinetics.net_production_rates[i])

            for i in range(0, len(self.rsurf.kinetics.net_production_rates)):
                results[surf_ROP_str[i]].append(self.rsurf.kinetics.net_production_rates[i])

            for i in range(0, len(self.rsurf.kinetics.net_rates_of_progress)):
                results[surfrxn_ROP_str[i]].append(self.rsurf.kinetics.net_rates_of_progress[i])

            if gasrxn_ROP_str:
                for i in range(0, len(self.r.kinetics.net_rates_of_progress)):
                    results[gasrxn_ROP_str[i]].append(self.r.kinetics.net_rates_of_progress[i])
